...
class Auth {
  ...
  static const String postLogoutUri =
      "nl.kurza.test-project.auth://post-logout";
  static const String discoveryUri =
      "https://identity.kurza.nl/.well-known/openid-configuration";
  ...
  static Future<AuthInformation> login() async {
    var response = await appAuth.authorizeAndExchangeCode(AuthorizationTokenRequest(
        clientId, redirectUri,
        discoveryUrl: discoveryUri, scopes: scopes));

    return AuthInformation.fromResponse(response, null, null);
  }

  static Future<AuthInformation> refresh(String refreshToken) async {
    var response = await appAuth.token(TokenRequest(clientId, redirectUri,
  ...
  static Future<AuthInformation> logout(String idToken) async {
    var response = await appAuth.endSession(EndSessionRequest(
        idTokenHint: idToken,
        postLogoutRedirectUrl: postLogoutUri,
        discoveryUrl: discoveryUri));
